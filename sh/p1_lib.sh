...
#############################################################################################################################################
export COMPILER_FOLDER="../install/boot"
...
export COMPILER_TYPE="standalone_linux"
...
export COMPILER_SETTING="-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ "
    ...
    -DCMAKE_CXX_COMPILER_ID=Clang -DCMAKE_C_COMPILER_ID=Clang -DCMAKE_CXX_COMPILER_VERSION=18.1.8 -DCMAKE_CXX_STANDARD_COMPUTED_DEFAULT=ON -DCMAKE_CXX_EXTENSIONS_COMPUTED_DEFAULT=ON"
# cmake bug to check compiler version so, -DCMAKE_CXX_COMPILER_VERSION=18.1.8

if [ $COMPILER_TYPE = "standalone_linux" ]; then

    export LD_LIBRARY_PATH=${COMPILER_FOLDER}/lib
    export FLAG="-nostdinc++ -isystem${COMPILER_FOLDER}/include/${TARGET_TRIPLE} -isystem${COMPILER_FOLDER}/${TARGET_TRIPLE}/include/c++/v1 -isystem${COMPILER_FOLDER}/include -isystem${COMPILER_FOLDER}/include/c++/v1 -isystem/usr/include -I${ADDITIONAL_INCLUDE} "
...
elif [ $COMPILER_TYPE = "standalone_win" ]; then
    export FLAG="-nostdinc++ -isystem${COMPILER_FOLDER}/include -isystem${COMPILER_FOLDER}/include/c++/v1 -isystem/usr/include -I${ADDITIONAL_INCLUDE} "
    export COMMON="-G Ninja -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR "
    export LINK_FLAG="${LINK_FLAG} -Wl,-L${COMPILER_FOLDER}/lib -Wl,-rpath,${COMPILER_FOLDER}/lib \
    -Wl,-stdlib=libc++ -Wl,-lc++abi  -Wl,-lm -Wl,-lc -Wl,-lgcc_s -Wl,-lgcc"
fi
...
-DLLVM_LIBSTDCXX_MIN=ON -DLLVM_LIBSTDCXX_SOFT_ERROR=ON -DHAVE_CXX_ATOMICS_WITH_LIB=ON \
-DHAVE_CXX_ATOMICS64_WITH_LIB=ON -D_WCHAR_H_CPLUSPLUS_98_CONFORMANCE_=ON"
...
cmake ../llvm-project/${CMAKE_START_MODULE}  -DCMAKE_TOOLCHAIN_FILE=../sh/linux_system_clang.cmake \
   -DCMAKE_BUILD_TYPE=Debug -DLLVM_INCLUDE_TESTS=FALSE  \
   -DLLVM_ENABLE_PROJECTS="libc;compiler-rt;clang" \
   ${TRIPLE_SETTING} \
   ${TARGET_SETTING} \
   ${BUILD_FIX_SETTING} \
   ${COMMON} \
   -DCMAKE_C_FLAGS="${FLAG} -fprofile-instr-generate=${GEN_FOLDER}/code-$(date +%Y-%m-%d-%H-%M-%S)-$$.profraw" \
   -DCMAKE_CXX_FLAGS="${FLAG} -fprofile-instr-generate=${GEN_FOLDER}/code-$(date +%Y-%m-%d-%H-%M-%S)-$$.profraw" \
   -DCMAKE_EXE_LINKER_FLAGS="${MIMALLOC_LINK_SETTING}" \  
   ${ENABLE_STATIC} \
   ${COMPILER_SETTING} \
   ${LIBUNWIND_OFF_SETTING} \
   ${LIBCXX_WITHOUT_UNWIND} \
   ${LIBC_SETTING} ${LIBC_COMPILER_RT_SETTING} \
