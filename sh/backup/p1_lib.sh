...
export COMPILER_SETTING="-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ "
    ...
    -DCMAKE_CXX_COMPILER_ID=Clang -DCMAKE_C_COMPILER_ID=Clang -DCMAKE_ASM_COMPILER_ID=Clang -DLLVM_COMPILER_CHECKED=TRUE"
# cmake bug to check compiler version so, -DLLVM_COMPILER_CHECKED=TRUE
...
cmake ../llvm-project/${CMAKE_START_MODULE}  \
   ...
   -DCMAKE_STATIC_LINKER_FLAGS="${LINK_FLAG}" -DCMAKE_EXE_LINKER_FLAGS="${LINK_EXE_FLAG} ${MIMALLOC_LINK_SETTING}" \
   ${ENABLE_STATIC} \
   ${COMPILER_SETTING} \
