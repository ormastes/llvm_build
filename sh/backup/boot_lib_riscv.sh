...
export EMBEDDIG_LIB_SETTING="${USE_EXISTING_TABLEGEN} -DCMAKE_CROSSCOMPILING=True -DCMAKE_SYSTEM_NAME=Generic -DLIBCXX_ENABLE_FILESYSTEM=OFF -DLIBCXX_ENABLE_LOCALIZATION=OFF -DLIBCXX_ENABLE_MONOTONIC_CLOCK=OFF -DLIBCXX_ENABLE_RANDOM_DEVICE=OFF -DLIBCXX_ENABLE_WIDE_CHARACTERS=OFF -DLIBCXX_ENABLE_TIME_ZONE_DATABASE=OFF -DLIBCXX_ENABLE_THREADS=OFF -DLIBCXX_ENABLE_SHARED=OFF -DLIBCXX_ENABLE_RTTI=OFF -DLIBCXXABI_ENABLE_SHARED=OFF -DLIBCXXABI_BAREMETAL=ON -DLIBCXXABI_HAS_PTHREAD_LIB=OFF -DLIBCXXABI_ENABLE_THREADS=OFF -DLLVM_INCLUDE_BENCHMARKS=OFF -DLIBCXX_INCLUDE_BENCHMARKS=OFF -DLLVM_INCLUDE_TOOLS=OFF ${COMPILER_RT_BAREMETAL_SETTING} -DHAVE_CXX_ATOMICS_WITHOUT_LIB=1 -DHAVE_CXX_ATOMICS64_WITHOUT_LIB=1  -DLLVM_INCLUDE_UTILS=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_INCLUDE_EXAMPLES=OFF  -DLLVM_INCLUDE_DOCS=OFF "

export TRIPLE_LIB_INSTALL_DIR=${INSTALL_DIR}/lib/${TARGET_TRIPLE}
export TRIPLE_INCLUDE_INSTALL_DIR=${INSTALL_DIR}/include/${TARGET_TRIPLE}
export RT_INSTALL="-DCOMPILER_RT_INSTALL_INCLUDE_DIR=${TRIPLE_INCLUDE_INSTALL_DIR} -DCOMPILER_RT_INSTALL_LIBRARY_DIR=${TRIPLE_LIB_INSTALL_DIR} "
export LIBCXX_INSTALL="-DLIBCXXABI_INSTALL_LIBRARY_DIR=${TRIPLE_LIB_INSTALL_DIR} -DLIBCXXABI_INSTALL_INCLUDE_DIR=${TRIPLE_INCLUDE_INSTALL_DIR} -DLIBCXX_INSTALL_LIBRARY_DIR=${TRIPLE_LIB_INSTALL_DIR} -DLIBCXX_INSTALL_INCLUDE_DIR=${TRIPLE_INCLUDE_INSTALL_DIR} "
...
cmake ../llvm-project/${CMAKE_START_MODULE}  \
   ...
   #   ${LIBC_CLANG_SETTING}  ${LIBC_SETTING} ${LIBC_COMPILER_RT_SETTING} \
