...
if [ $IS_LLD -eq 1 ]; then
    ...
    export LINKER_SETTING="${LLD_SETTING} ${LIKER_SETTING}"
else
    export LINK_EXE_FLAG="" 
    export LINKER_SETTING=""
fi
...
export LLDB_SETTING="-DHAVE_LIBCOMPRESSION=OFF"
...
export EXISTING_TABLEGEN="-DLLVM_EXTERNAL_TABLEGEN=ON -DLLVM_TABLEGEN=/usr/bin/llvm-tblgen"
...
if [ $IS_BAREMETAL -eq 1 ]; then
    export LIBC_SETTING=" ${LIBC_COMPILER_RT_SETTING} \
        -DLLVM_LIBC_FULL_BUILD=ON \
        -DLLVM_LIBC_INCLUDE_SCUDO=OFF \
        -DLIBC_INCLUDE_DOCS=ON"
    export LIBUNWIND_OFF=" -DLLVM_ENABLE_SPHINX=OFF -DLIBUNWIND_INCLUDE_DOCS=OFF"  # pip install sphinx  
    export EMBEDDIG_LIB_SETTING="${EXISTING_TABLEGEN} -DCMAKE_CROSSCOMPILING=True -DCMAKE_SYSTEM_NAME=Generic -DLIBCXX_ENABLE_FILESYSTEM=OFF -DLIBCXX_ENABLE_LOCALIZATION=OFF -DLIBCXX_ENABLE_MONOTONIC_CLOCK=OFF -DLIBCXX_ENABLE_RANDOM_DEVICE=OFF \
    ...
    ${COMPILER_RT_BAREMETAL} -DHAVE_CXX_ATOMICS_WITHOUT_LIB=1 -DHAVE_CXX_ATOMICS64_WITHOUT_LIB=1  -DLLVM_INCLUDE_UTILS=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_INCLUDE_EXAMPLES=OFF  -DLLVM_INCLUDE_DOCS=OFF "
else
    export LIBC_SETTING=" ${LIBC_COMPILER_RT_SETTING} \
        -DLLVM_LIBC_FULL_BUILD=ON \
        -DLLVM_LIBC_INCLUDE_SCUDO=ON \
        -DLIBC_INCLUDE_DOCS=ON"
    export LIBUNWIND_OFF=" -DLLVM_ENABLE_SPHINX=ON -DLIBUNWIND_INCLUDE_DOCS=OFF"  # pip install sphinx  
    export EMBEDDIG_LIB_SETTING=""
fi

if [ $IS_LIBC -eq 0 ]; then
    export LIBC_SETTING=""
fi
...
export CLANG_RT_SETTING="-DCLANG_DEFAULT_RTLIB=compiler-rt"
