...
export OS_INCLUDE="-isystem/usr/include -isystem/usr/include/x86_64-linux-gnu"
export ADD_PATH=

export COMPILER_FOLDER=../install/boot1
...
export ENABLE_STATIC="-DCMAKE_POSITION_INDEPENDENT_CODE=ON -DBUILD_SHARED_LIBS=OFF -DLLVM_BUILD_STATIC=ON -DLIBCXXABI_ENABLE_STATIC=ON -DLIBCXX_ENABLE_STATIC=ON -DCOMPILER_RT_STATIC_CXX_LIBRARY=ON \
...
# libc/utils/LibcTableGenUtil/CMakeLists.txt > # if LibcTableGenUtil is not already added to target
# if (NOT TARGET LibcTableGenUtil)
...
cmake ../llvm-project/${CMAKE_START_MODULE} ${COMMON}  \
   -DCMAKE_BUILD_TYPE=Debug  \
   -DCMAKE_TOOLCHAIN_FILE=../sh/linux_system_clang.cmake \
   -DLLVM_ENABLE_RUNTIMES="compiler-rt;libc;llvm-glibc;libcxx;libcxxabi;libunwind" \
   ${LIBC_CLANG_SETTING}  
