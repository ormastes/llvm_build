...
export BUILD_BAREMETAL_FIX_SETTING="-DLLVM_LIBSTDCXX_MIN=OK -DLLVM_LIBSTDCXX_SOFT_ERROR=OK -DHAVE_CXX_ATOMICS64_WITHOUT_LIB=ON -DHAVE_CXX_ATOMICS_WITHOUT_LIB=ON -DHAVE_CXX_ATOMICS_WITH_LIB=OFF \
-DHAVE_CXX_ATOMICS64_WITH_LIB=OFF -D_WCHAR_H_CPLUSPLUS_98_CONFORMANCE_=ON \
-DHAVE_STEADY_CLOCK=OFF -DHAVE_POSIX_REGEX=OFF -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON -DCXX_SUPPORTS_CUSTOM_LINKER=OK "


export COMPILER_FOLDER="/usr/share/gcc-arm-none-eabi-13.3.rel1"
export LD_LIBRARY_PATH=${COMPILER_FOLDER}/lib
export FLAG="-nostdinc++ -isystem${COMPILER_FOLDER}/${TARGET_TRIPLE}/include/c++/13.3.1 -isystem${COMPILER_FOLDER}/${TARGET_TRIPLE}/include " # -isystem${COMPILER_FOLDER}/include -isystem${COMPILER_FOLDER}/include/c++/13.3.1 -isystem/usr/include -I${ADDITIONAL_INCLUDE} "
export LINK_FLAG="${LINK_FLAG} -Wl,-L${COMPILER_FOLDER}/${TARGET_TRIPLE}/lib -Wl,-rpath,${COMPILER_FOLDER}/${TARGET_TRIPLE}/lib -Wl,nostdlib -Wl,-nodefaultlibs -Wl,-nostartfiles -Wl,-lc "

export LINK_EXE_FLAG="${LINK_FLAG}" #"-fuse-ld=lld  -Wl,--threads=8 ${LINK_FLAG}" # -threads need LLD enable

# /usr/share/gcc-arm-none-eabi-13.3.rel1/arm-none-eabi/include /usr/share/gcc-arm-none-eabi-13.3.rel1/arm-none-eabi/include/c++/13.3.1
# /usr/share/gcc-arm-none-eabi-13.3.rel1/arm-none-eabi/lib 

cmake ${COMMON}  ../llvm-project/${CMAKE_START_MODULE} -DCMAKE_BUILD_TYPE=Debug  -DLLVM_INCLUDE_TESTS=FALSE -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON  \
  -DLLVM_ENABLE_PROJECTS="compiler-rt" \
  -DCMAKE_C_COMPILER=arm-none-eabi-gcc \
  -DCMAKE_CXX_COMPILER=arm-none-eabi-g++ \
  -DCMAKE_ASM_COMPILER=arm-none-eabi-gcc -DCMAKE_C_COMPILER_TARGET="ARM" -DCMAKE_CXX_FLAGS="$FLAG" -DCMAKE_C_FLAGS="$FLAG" \
    -DCMAKE_AR=arm-none-eabi-ar \
    -DCMAKE_RANLIB=arm-none-eabi-ranlib \
    -DCMAKE_LINKER=arm-none-eabi-ld \
  -DLLVM_TARGETS_TO_BUILD="ARM" \
  -DLIBC_TARGET_ARCHITECTURE="armv7e-m" \
  -DLIBC_TARGET_TRIPLE="arm-none-eabi" -DLIBC_TARGET_OS=baremetal -DLLVM_ENABLE_THREADS=OFF -DLLVM_COMPILER_IS_GCC_COMPATIBLE=FALSE -DHAVE_CXX_ATOMICS64_WITHOUT_LIB=OFF -DHAVE_CXX_ATOMICS_WITHOUT_LIB=OFF -DLIBC_TARGET_OS_IS_BAREMETAL=ON -DLLVM_INCLUDE_BENCHMARKS=OFF -DCMAKE_SYSTEM_NAME=Generic ${COMPILER_RT_BAREMETAL_SETTING} -DHAVE_SYSEXITS_H=OFF -DLLVM_ENABLE_THREADS=OFF -DLLVM_LIBS=OFF \
  -DLIBC_BAREMETAL_BUILD=ON 

#-DCMAKE_HAVE_LIBC_PTHREAD=OFF -DLLVM_INCLUDE_TESTS=FALSE -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON 
# -DHAVE_CXX_ATOMICS64_WITHOUT_LIB=OFF -DHAVE_CXX_ATOMICS_WITHOUT_LIB=OFF
    #libc.src.compiler.__stack_chk_fail      #libc.src.stdlib.abort
