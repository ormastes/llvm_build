...
cmake ${COMMON}  ../llvm-project/${CMAKE_START_MODULE} -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON -DCMAKE_BUILD_TYPE=Debug -DCMAKE_HAVE_LIBC_PTHREAD=OFF -DLLVM_INCLUDE_TESTS=FALSE  \
  -DLLVM_ENABLE_RUNTIMES="libc" -DLLVM_ENABLE_LIBCXX=ON \
  -DCMAKE_C_COMPILER=clang \
  -DCMAKE_CXX_COMPILER=clang++ \
  -DCMAKE_CXX_COMPILER=clang++ \
  ${LLVM_ENABLE_LLD} ${BUILD_BAREMETAL_FIX_SETTING} \
  -DCMAKE_C_COMPILER_TARGET=arm-none-eabi \
  -DCMAKE_CXX_COMPILER_TARGET=arm-none-eabi \
  -DLLVM_TARGETS_TO_BUILD="ARM" \
  -DLIBC_TARGET_ARCHITECTURE="armv7e-m" \
  -DLIBC_TARGET_TRIPLE="arm-none-eabi" -DLIBC_TARGET_OS=baremetal -DLIBC_TARGET_OS_IS_BAREMETAL=ON -DLLVM_INCLUDE_BENCHMARKS=OFF \
