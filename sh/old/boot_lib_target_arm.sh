...
export PROJECT=boot_baremetal_arm
...
export RUNTIME_TARGET_TRIPLE=${ARM_BAREMETAL_TARGET_TRIPLE}
...
if [ $IS_BAREMETAL -eq 1 ]; then
    ...
    export BAREMETAL_SETTING="-DLIBCXXABI_BAREMETAL=ON -DLIBCXX_BAREMETAL=ON -DLLVM_INCLUDE_BENCHMARKS=OFF \
...
export RUNTIME_TRIPLE_SETTING="-DLLVM_RUNTIME_TARGETS='${RUNTIME_TARGET_TRIPLE}' "
...
export FUSE_LD_LLD="" # -fuse-ld=lld"
export LINK_FLAG="" # -threads need LLD enable
export LINK_EXE_FLAG="${FUSE_LD_LLD} -Wl,--threads=8 ${LINK_FLAG}" # -threads need LLD enable
...
export COMPILER_RT_BAREMETAL="-DCOMPILER_RT_BUILD_BUILTINS=ON \
    -DCOMPILER_RT_HAS_SANITIZER_COMMON=OFF -DCOMPILER_RT_BUILD_SANITIZERS=OFF -DCOMPILER_RT_BUILD_XRAY=OFF -DCOMPILER_RT_BUILD_MEMPROF=OFF  \
...
PROFILE_FOLDER=~/dev/llvm/llvm_pgo_profile
GEN_FOLDER=${PROFILE_FOLDER}/pgo_gen
...
# -DLIBCXX_INCLUDE_BENCHMARKS=OFF -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON 
# -DLLVM_LIBSTDCXX_MIN=ON -DLLVM_LIBSTDCXX_SOFT_ERROR=ON 
...
-DHAVE_STEADY_CLOCK=OFF -DHAVE_POSIX_REGEX=OFF -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON"
...
cmake ../llvm-project/${CMAKE_START_MODULE}   -DCMAKE_TOOLCHAIN_FILE=../sh/linux_system_clang_nolld.cmake \
   -DCMAKE_BUILD_TYPE=Debug -DLLVM_INCLUDE_TESTS=FALSE  \
   -DLLVM_ENABLE_PROJECTS="clang;lld" \
   -DLLVM_ENABLE_RUNTIMES="libc;compiler-rt" \
   ${BAREMETAL_SETTING} ${BUILD_FIX_SETTING} ${COMPILER_RT_BAREMETAL} ${BUILD_BAREMETAL_FIX_SETTING} \
   ${COMMON} \
   -DHAVE_STEADY_CLOCK=OFF -DHAVE_POSIX_REGEX=OFF -DLLVM_LIBC_FULL_BUILD=ON \
   ${TRIPLE_SETTING} \
   -DCMAKE_EXE_LINKER_FLAGS="${MIMALLOC_LINK_SETTING}" \
   ${LIBC_SETTING} 
   
   
   
# LIBC_COMPILER_RT_SETTING   COMPILER_RT_BAREMETAL  LIBC_CLANG_SETTING
