...
export PROJECT=boot
export BIN_BUILD=0
export ADDITIONAL_INCLUDE=../llvm-project/llvm/include # libc build bug fix
export ADD_PATH=

export COMPILER_FOLDER=../install/${PROJECT}
...
if [ -z "$COMPILER_FOLDER" ]; then
    echo "COMPILER_FOLDER is not set"
else
...
if [ -z "$COMPILER_FOLDER" ]; then
    export COMPILER_SETTING="-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_ASM_COMPILER=clang   -DCMAKE_RANLIB=llvm-ranlib -DCMAKE_LINKER=ld.lld \
        -DCMAKE_CXX_COMPILER_ID=Clang -DCMAKE_C_COMPILER_ID=Clang  -DCMAKE_ASM_COMPILER_ID=Clang "
    export FLAG=""
...
COMPILER_BIN_FOLDER=$COMPILER_FOLDER/bin
    ...
    -DCMAKE_AR=${COMPILER_BIN_FOLDER}/llvm-ar -DCMAKE_RANLIB=${COMPILER_BIN_FOLDER}/llvm-ranlib -DCMAKE_LINKER=${COMPILER_BIN_FOLDER}/ld.lld \
    -DCMAKE_CXX_COMPILER_ID=Clang -DCMAKE_C_COMPILER_ID=Clang  -DCMAKE_ASM_COMPILER_ID=Clang "

    export LD_LIBRARY_PATH=${COMPILER_FOLDER}/lib
    export FLAG="-nostdinc++ -isystem${COMPILER_FOLDER}/include/${TARGET_TRIPLE}/c++/v1 -isystem${COMPILER_FOLDER}/include -isystem${COMPILER_FOLDER}/include/c++/v1 -isystem/usr/include -I${ADDITIONAL_INCLUDE} "
    export COMMON="-G Ninja -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR "
    export LINK_FLAG="${LINK_FLAG} -Wl,-L${COMPILER_FOLDER}/lib -Wl,-rpath,${COMPILER_FOLDER}/lib \
    -Wl,-lc++ -lc++abi -lllvmlibc "
...
if [ -z "$ADD_PATH"]; then
    ...
    export LINK_EXE_FLAG="${LINK_EXE_FLAG} -Wl,-L${ADD_PATH}/lib -Wl,-rpath,${ADD_PATH}/lib \
        -Wl,-stdlib=libc++ test.cpp -lc++ -lc++abi -lllvmlibc  "
...
export COMPILER_RT_SETTING="-DCOMPILER_RT_BUILD_BUILTINS=ON -DCOMPILER_RT_DEFAULT_TARGET_ONLY=ON -DCOMPILER_RT_BUILD_SHARED_LIBS=OFF"
...
export COMPILER_CHECK_DISABLE=" -DLLVM_INCLUDE_TESTS=FALSE -DCMAKE_C_COMPILER_WORKS=ON -DCMAKE_CXX_COMPILER_WORKS=ON"
...
cmake ../llvm-project/${CMAKE_START_MODULE} ${COMPILER_SETTING} \
   -DCMAKE_BUILD_TYPE=Debug -DLLVM_INCLUDE_TESTS=FALSE  ${COMPILER_CHECK_DISABLE} -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON ${VERBOSE_SETTING} \
